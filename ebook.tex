% !TeX program = XeLaTeX
% !TeX encoding = UTF-8
\documentclass[fontsize=11pt,oneside,a5paper]{scrbook}

\usepackage[USenglish]{babel}
\usepackage{xunicode}
\usepackage{xltxtra}
\usepackage{xeCJK}
\setCJKmainfont{IPAexMincho}
\usepackage{lmodern}
\usepackage{xspace}
\usepackage{minted}
\usepackage{listings}
\usepackage{spverbatim}
\usepackage{amssymb}
\usepackage[autostyle=true]{csquotes}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{url}
\usepackage{float}
\usepackage[pdfauthor={Darth-Revan},pdftitle={The Rust Programming Language},pdfcreator={XeLaTeX}]{hyperref}
\hypersetup{colorlinks,urlcolor=blue,linkcolor=blue,citecolor=black}
\usepackage[most]{tcolorbox}
\usepackage{fancyvrb,newverbs,mdframed}
\usepackage[automark,headsepline,footsepline,markuppercase]{scrpage2}

\definecolor{cverbbg}{gray}{0.95}
\definecolor{quote-blue}{rgb}{0.95,0.97,0.98}
\newtcolorbox{myquote}{colback=quote-blue,grow to right by=-10mm,grow to left by=-10mm,
    boxrule=0pt,boxsep=0pt,breakable}
    
\makeatletter
\def\@xobeysp{\mbox{}\space}
\def\verbatim@font{\footnotesize\ttfamily\raggedright}
\makeatother

\let\oldsection\section
\renewcommand{\section}{\clearpage\oldsection}

\newcommand{\parag}[1]{\paragraph*{#1}\mbox{}\\}
\newcommand{\subparag}[1]{\subparagraph*{#1}\mbox{}\\}

\begin{document}

    \setromanfont{Liberation Serif}
    \setsansfont{Liberation Sans}
    \setmonofont{Source Code Pro}

    \renewcommand{\arraystretch}{1.4}
%     \renewenvironment{verbatim}
%     {\VerbatimEnvironment
%      \begin{mdframed}[backgroundcolor=cverbbg,topline=false,bottomline=false,rightline=false,leftline=false]
%      \begin{Verbatim}[fontsize=\footnotesize]}
%     {\end{Verbatim}
%      \end{mdframed}}

    \setminted[bash]{
      breaklines=true,
      samepage=false,
      fontsize=\footnotesize,
      tabsize=4,
      encoding=utf-8,
      outencoding=utf-8,
    }

    \renewenvironment{verbatim}
    {\VerbatimEnvironment
     \begin{mdframed}[backgroundcolor=cverbbg,topline=false,bottomline=false,rightline=false,leftline=false]
     \begin{minted}{bash}}
    {\end{minted}
     \end{mdframed}}
    
    \usemintedstyle{manni}
    \setminted[rust]{
      breaklines=true,
      samepage=false,
      fontsize=\footnotesize,
      tabsize=4,
      encoding=utf-8,
      outencoding=utf-8,
    }
    
    \newenvironment{rustc}
    {\VerbatimEnvironment
     \vspace{0.5em}
     \begin{mdframed}[backgroundcolor=cverbbg,topline=false,bottomline=false,rightline=false,leftline=false]
     \begin{minted}{rust}}
    {\end{minted}
     \end{mdframed}
     \vspace{0.5em}}
    
    \newcommand{\code}[1]{\texttt{\small \textcolor{red!60!black}{#1}}}
    \newcommand{\blank}{\vspace{\baselineskip}}
    \newcommand{\myparagraph}[1]{\paragraph{#1}\mbox{}\\}
    
    \newcommand{\blankpage}{\pagebreak\null\newpage}
    
    % A few macros for emphasized variable names and keywords
    \newcommand{\x}{\code{x}}
    \newcommand{\y}{\code{y}}
    \newcommand{\z}{\code{z}}
    \newcommand{\struct}{\code{struct}}
    \newcommand{\mut}{\code{mut}}
    \newcommand{\varchar}{\code{char}}
    \newcommand{\println}{\code{println!}}
    \newcommand{\panic}{\code{panic!}}
    \newcommand{\keylet}{\code{let}}
    \newcommand{\enum}{\code{enum}}
    \newcommand{\String}{\code{String}}
    \newcommand{\match}{\code{match}}
    \newcommand{\keyref}{\code{ref}}
    \newcommand{\itt}{\code{i32}}
    \newcommand{\keyif}{\code{if}}
    \newcommand{\result}{\code{Result}}
    \newcommand{\option}{\code{Option}}
    \newcommand{\none}{\code{None}}
    
    \clearscrheadfoot
    \cfoot[Page \pagemark]{Page \pagemark}
    \ohead{\rightmark}
    \automark[subsection]{section}
    \pagestyle{scrheadings}
    
    \include{src/title}
    \pagebreak
    
    \tableofcontents
    \pagebreak

    \blankpage
    \chapter{Introduction}
    \label{sec:intro}
    \input{src/introdcution.tex}
    
    \blankpage
    \chapter{Getting Started}
    \label{sec:gettingstarted}
    \input{src/gettingstarted.tex}
    
    \blankpage
    \chapter{Tutorial: Guessing Game}
    \label{sec:tutorial}
    \input{src/tutorial.tex}
    
    \blankpage
    \chapter{Syntax and Semantics}
    \label{sec:syntax}
    \input{src/syntax.tex}
    
    \blankpage
    \chapter{Effective Rust}
    \label{sec:effective_rust}
    \input{src/effective_rust.tex}
    
    \blankpage
    \chapter{Nightly Rust}
    \label{sec:nightly_rust}
    \input{src/nightly_rust.tex}
    
    \blankpage
    \chapter{Glossary}
    \label{sec:glossary}
    \input{src/glossary.tex}
    
    \blankpage
    \chapter{Syntax Index}
    \label{sec:syntax_index}
    
    \blankpage
    \chapter{Bibliography}
    \label{sec:bib}
    \input{src/bibliography.tex}
    
\end{document}
