...
\subsection*{Background: \code{Send} and \code{Sync}}
...
In the opposite way, if we were wrapping a library with FFI (see \nameref{sec:effective_FFI}) that isn't threadsafe, we wouldn't want to 
implement \code{Send}, and so the compiler will help us enforce that it can't leave the current thread.
...
\nameref{sec:syntax_mutability}) are inherently \code{Sync}, which includes simple primitive types (like \code{u8}) and aggregate 
...
For that, we can use the \code{Mutex<T>} type! Here's the working version:
