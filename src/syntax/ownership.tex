...
% TODO make borrowing and lifetimes hyperrefs
\begin{itemize}
...
% TODO make heap a hyperref
When \code{v} comes into scope, a new [vector] is created, and it allocates space on the heap for each of its elements. When \code{v}
goes out of scope at the end of \code{foo()}, Rust will clean up everything related to the vector, even the heap-allocated memory. 
This happens deterministically, at the end of the scope.

\blank

% TODO make vectors a hyperref
...
heap at runtime. They behave like arrays, except their size may change by \code{push()}ing more elements onto them.

\blank

% TODO make generic type a hyperref
Vectors have a generic type \code{Vec<T>}, so in this example \code{v} will have type \code{Vec<i32>}. We'll cover generics 
...
% TODO make stack and heap hyperrefs
...
We will discuss how to make your own types \code{Copy} in the traits section.
